...
var ids = ['thumbnail-list-view', 'thumbnail-list-title',
           ...
           'thumbnails-single-info-button', 'info-view', 
...
//softkey bar & option menu items
var optTakeVideo = {                // create array of required actions to be passed to softkeys&options menu
      /*l10nId: "",*/
	  text: "Take video",             // text OR <img> tag to be placed to SK button
      optionId: "idLaunchCamera", // Unique ID of action (will be returned in callback)
      priority: 2,                // priority of action: 1=LSK, 2=CSK, 5... = Context Menu items
      disabled: false              // if the action shall be disabled	  	  
    };
    
var optLaunchCamera = {      
	  text: "Take video",
      optionId: "idLaunchCamera",
      priority: 1,
      disabled: false
    };
...
var optRotate = {
      text: "Rotate Video CW",
      optionId: "idRotate",
      priority: 6,
      disabled: false
    };*/

var optPlayFullScreen = {
      text: "Full screen",
      optionId: "idFullScreen",
      priority: 1,
      disabled: false
    };
...
var optPlay = {
      text: "<img id='csk-play-pause' src='style/images/tablet_pause.png'/>",
      optionId: "idPlay",
      priority: 2,
      disabled: false
    };
...
var skCurrent;	
var skMainList = [optLaunchCamera, optDelete, optRename, optShare, optMultiSelect, optFileInfo, optLock];
var skMainListLocked = [optLaunchCamera, optRename, optShare, optMultiSelect, optFileInfo, optUnlock];
var skPlayer = [optPlayFullScreen, optPlay, optShare, optFileInfo, optDelete];
var skRename = [optCancel, optSave];
...
function deleteSelectedItems() {
  ...
  /*get the unlocked item, locked file can not be deleted*/  
  unlockedItems = selectedFileNames.filter(function(filename) {
     return !thumbnailList.thumbnailMap[filename].data.metadata.locked;
  });
...
function setControlsVisibility(visible) {
  ...
  dom.softKeyBar.hidden = !controlShowing;
...
//soft key bar and option menu handler
window.addEventListener('softkeys-callback-event', function(e) {
  
  function setLocked(item, locked) {        
    item.data.metadata.locked = locked;
    item.setLocked(locked);

    // Save the new metadata to the db, but don't wait for it to complete
    videodb.updateMetadata(item.data.name, item.data.metadata);      
  }
      
  console.log('softkeys-callback-event: ' + e.detail.id);
  ...
  switch(e.detail.id) {
    ...
    case "idMultiSelect":
      showSelectView();
      setSoftKeyBar([optSelectAll]);
      break
  
    /*share option*/
    case "idShare":
      //share (multi)selected item(s) in selection view
      if (currentLayoutMode === LAYOUT_MODE.selection) {
        shareSelectedItems();
      }
      //share current video 
      else {
    ...
    case "idDeselectAll":
    case "idSelectAll":
      var select = (e.detail.id === "idSelectAll") ? true : false;
      selectedFileNames = [];
      selectedFileNamesToBlobs = {};
      
      FocusablesFilter._controls.forEach( function (thumbnail) {                
        if (select  === true) { //select all
          var filename = thumbnail.data.name;
    ...
    case "idUnlock":      
    case "idLock":    
      var locked = (e.detail.id === "idLock") ? true : false;
            
      if (currentLayoutMode === LAYOUT_MODE.list) {        
        var thumbnail = FocusablesFilter._controls[FocusablesFilter._curIndex];        
        
        setLocked(thumbnail, locked);     
      }
      else if (currentLayoutMode === LAYOUT_MODE.selection) {
        selectedFileNames.forEach(function(filename) {
          setLocked(thumbnailList.thumbnailMap[filename], locked);
        });
      }
    ...
    case "idRename":
        ...
        setSoftKeyBar(skRename);
  	    dom.fileRenameView.classList.remove('hidden');
  	    });
      break;
    ...
    case "idCancel":
      ...
      location.replace(location);
  	  dom.fileRenameView.classList.add('hidden');
  	  var rename = document.getElementById('renameId');
      rename.setAttribute('value','');
  	  setSoftKeyBar(skMainList);
    ...
    case "idSave":
      console.log("save rename.");
  	  var rename = document.getElementById('renameId');
  	  currentVideo.metadata.title=rename.value;
  	  // Save the new metadata to the db, but don't wait for it to complete
...
window.addEventListener('hw-keyup', (e) => {
  if ((dom.softKeyBar._menuVisible == false) && (e.detail.key == "Back")) {
    if (currentLayoutMode === LAYOUT_MODE.selection) {
      hideSelectView();
      setSoftKeyBar();
    }
	else if (currentLayoutMode === LAYOUT_MODE.fullscreenPlayer) {
      handleCloseButtonClick();      
    }
  }  
});
...
window.addEventListener('softkey-up', (e) => {
  if ((currentLayoutMode === LAYOUT_MODE.fullscreenPlayer) && controlShowing === false ) {
    if ((e.detail.key == "RSK") || (e.detail.key == "CSK")) {
      setControlsVisibility(true);  //quit full screen mode
    }
  }  
});

window.addEventListener('focus-changed', (e) => {
  console.log('focus-changed: ' + e.detail.from + ' --> ' + e.detail.to);
  
  if (currentLayoutMode === LAYOUT_MODE.list) {
    currentVideo = FocusablesFilter._controls[FocusablesFilter._curIndex].data;
    setSoftKeyBar();
  }
  else if (currentLayoutMode === LAYOUT_MODE.selection) {
    //update soft bar after deletion, 
    //the deletion of thumbnail item is done in videodb.ondeleted, it is asynchronous
    //it's better to add new event to indicate focus filter to reset
    setSoftKeyBar();  
  }
});

window.addEventListener('moz-content-interactive', (e) => {
  console.log('ddts moz-content-interactive');
});

function setSoftKeyBar(skOptions) {
  console.log('setSoftKeyBar:  skOption: ' + skOptions + '  currentLayoutMode: ' + currentLayoutMode);
  
  /*undefined: Softkey bar is no specified, update option items for each view*/
  if (skOptions === undefined) { 
    if (currentLayoutMode === LAYOUT_MODE.list) {
      if ( currentVideo ) {              
        skOptions = currentVideo.metadata.locked ? skMainListLocked : skMainList;
      }
      else {
        skOptions = skMainList;
      }
    }
      ...
      skOptions = (selectedFileNames.length === FocusablesFilter.getSize()) ? 
        [optDeselectAll] : [optSelectAll];
      
      if (selectedFileNames.length > 0) { /*no selected items, no RSK*/
        selectedFileNames.forEach(function(filename) {
          if ( thumbnailList.thumbnailMap[filename].data.metadata.locked ) {
            lockedCount++;
          }
        });
      
        /*all selected items are locked: no deletion*/
        if ( lockedCount === selectedFileNames.length ) {
          skOptions.push(optShare, optUnlock);
        }
        else {
          skOptions.push(optDelete, optShare, optLock);
        }
      }
    }
      ...
      skOptions = skPlayer;
      if ( currentVideo.metadata.locked ) {
        skOptions.push(optDelete);
      }
    }    
  }
  
  var skh = new SoftkeysHelper(skOptions);  //create SKHelper
  skCurrent = skOptions;
  skh.initActions();  
}
...
window.addEventListener('keyup', (e) => {
  ...
  if(e.key == 'Left' || e.key == 'ArrowLeft') {
    seekVideo(dom.player.currentTime - 10);
  }
...
function updatePlayIcon(paused) {
  var cskIcon = dom.softKeyBar.csk.querySelector('#csk-play-pause');
  
  if (cskIcon) {  
    if (paused) {
      cskIcon.src = 'style/images/tablet_play.png';
    }
    else {
      cskIcon.src = 'style/images/tablet_pause.png';
    }
  }
