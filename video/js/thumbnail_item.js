 ...
 *   setLocked: set this thumbnail item as locked or not. 
 *              A locked item can not be deleted. args:
 *              locked: a boolean for locked or not.
 *
 * Properties:
 ...
 *   lockedNode: the HTML DOM node for locked marker. This property may be
 *               null if template doesn't supply one.
...
function ThumbnailItem(videoData) {
  ...
  // the dom element for indicating a video is locked or not.
  ...
  function convertToDOM(htmlText) {
    ...
    _this.lockedNode = domNode.querySelector('.locked');
  ...
  function render() {
    ...
    _this.setLocked(_this.data.metadata.locked);
...
ThumbnailItem.prototype.setLocked = function(locked) {
  if (!this.lockedNode) {
    return;
  }
  this.lockedNode.hidden = !locked;
};
